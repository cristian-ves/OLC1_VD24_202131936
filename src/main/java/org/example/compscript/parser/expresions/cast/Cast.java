        ...
        switch (type.getType()) {
            case DOUBLE -> {
                return switch (expType) {
                    ...
                    default -> new CompError(ErrorType.RUNTIME, "Invalid cast type " + expType.getValue() + " to double", line, column);
                };
            }
            case WHOLE -> {
                return switch (expType) {
                    ...
                    default -> new CompError(ErrorType.RUNTIME, "Invalid cast type " + expType.getValue() + " to int", line, column);
                };
            }
            case CHAR -> {
                return (expType == dataType.WHOLE)
                        ? (char) ((int) res) :
                        new CompError(ErrorType.RUNTIME, "Invalid cast type " + expType.getValue() + " to char", line, column);
            }
            default -> {
                return new CompError(ErrorType.RUNTIME, "Invalid cast types " + expType.getValue() + " to " + type.getType().getValue(), line, column);
            }
        }
    }
}
